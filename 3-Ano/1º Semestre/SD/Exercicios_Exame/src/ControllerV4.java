...
public class ControllerV4 implements Controlador {

    private int terminal = -1;
    private int ocupacao = 0;
    private int[] pdestino = {0,0,0,0,0};
    private int[] pespera = {0,0,0,0,0};
    private int[] bemitido = {1,1,1,1,1};
    private int[] busados = {0,0,0,0,0};
    ...
    public void requisita_viagem(int origem, int destino) throws InterruptedException {
        l.lock();
        int bilhete = bemitido[origem];
        bemitido[origem]++;
        pespera[origem]++;
        while(origem != terminal || bilhete > (busados[origem] + 30-ocupacao)){ // espero se o autocarro não estiver no meu terminal ou se o meu bilhete não está dentro do intervalo de bilhetes possíveis para aquela viagem
            espera_entrar.await();
        }
        busados[origem]++;
        ocupacao++;
